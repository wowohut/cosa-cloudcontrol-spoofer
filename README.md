...
CosaSpoof 是一个基于 `libxposed API 102` 开发的 Xposed 模块，专为 ColorOS/realmeUI 系统设计。其核心功能是拦截并修改系统「应用增强服务」(`com.oplus.cosa`) 读取的 `ro.boot.prjname`（主板机型代号）属性，将其伪装为用户指定的机型代号。
...
- **开发架构**：使用 Kotlin 开发，设置界面基于 Jetpack Compose 构建，并采用 `libxposed API 102` 。
